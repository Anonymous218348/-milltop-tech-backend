...
const updateOrCreateStore = async (userId, domain, result) => {
  ...
  const domainClean = domainFromUrl(domain);

  // Try to find existing store by exact URL or domain match
  const existing = await db.query(
    `SELECT * FROM stores 
     WHERE user_id = $1 AND (url = $2 OR url LIKE $3 OR url LIKE $4)
     ORDER BY created_at DESC LIMIT 1`,
    [userId, url, `%${domainClean}%`, `%${domain}%`]
  );
  ...
  // Create new store entry with just email info
  const { rows } = await db.query(
  ...
  asyncHandler(async (req, res) => {
    ...
    const results = await Promise.all(
      req.body.domains.map(async (domain) => {
        ...
        return updateOrCreateStore(req.user.id, domain, result);
      })
    );
